            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form id="filterReviewsForm" action="{{ route('admin.content.reviews.index') }}" method="GET"
                            class="mb-4">
                            <div class="row g-3">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-auto">
                                    ...
                                    <a href="{{ route('admin.content.reviews.index') }}" class="btn btn-secondary">Đặt
                                        lại</a>
                                </div>
                            </div>
                        </form>
                                ...
                                <tbody>
                                    @forelse($reviews as $review)
                                        <tr id="review-row-{{ $review->id }}">
                                            <td>{{ $review->id }}</td>
                                            <td>{{ $review->customer->name ?? 'N/A' }}
                                                ({{ $review->customer->email ?? 'Khách vãng lai' }})</td>
                                            <td>{{ $review->product->name ?? 'Sản phẩm đã xóa' }}</td>
                                            <td>
                                                @for($i = 0; $i < $review->rating; $i++)<i
                                                class="bi bi-star-fill text-warning"></i>@endfor
                                                @for($i = $review->rating; $i < 5; $i++)<i
                                                class="bi bi-star text-warning"></i>@endfor
                                            </td>
                                            ...
                                            <td>
                                                <button class="btn btn-info btn-sm view-review-btn" data-bs-toggle="modal"
                                                    data-bs-target="#viewReviewModal" data-id="{{ $review->id }}"
                                                ...
                                                <div class="dropdown d-inline-block">
                                                    <button class="btn btn-primary btn-sm dropdown-toggle" type="button"
                                                        id="dropdownMenuButton-{{ $review->id }}" data-bs-toggle="dropdown"
                                                        aria-expanded="false">
                                                    ...
                                                    <ul class="dropdown-menu"
                                                        ...
                                                        <li><a class="dropdown-item text-danger delete-review-btn" href="#"
                                                                ...
                                                                data-customer-name="{{ $review->customer->name ?? 'N/A' }}"
                                                                data-product-name="{{ $review->product->name ?? 'Sản phẩm đã xóa' }}">Xóa</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
