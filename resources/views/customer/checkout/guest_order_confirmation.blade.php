            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="text-center">Đơn hàng của bạn **#{{ $order->formatted_id }}** đã được tiếp nhận và đang
                            chờ xử lý.
                        </p>
                        <hr>
                        ...
                        <ul class="list-group mb-3">
                            <li class="list-group-item d-flex justify-content-between lh-sm">
                                ...
                                <span class="text-muted">#{{ $order->formatted_id }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between lh-sm">
                                <div>
                        ...
                        @elseif($order->isRetriable()) {{-- Nếu đơn hàng có thể thanh toán lại (PENDING/FAILED/CANCELLED +
                            Online Method) --}}
                            @php
                            ...
                            @if($paymentRetryRoute)
                                <div class="alert alert-warning mt-4 text-center">
                                    <p class="mb-2">Đơn hàng này đang ở trạng thái **{{ $order->status_text }}** và chờ hoàn tất
                                        thanh toán trực tuyến.</p>
                                    <a href="{{ $paymentRetryRoute }}" class="btn btn-primary">
                                        <i class="bi bi-wallet-fill"></i> Thanh toán lại ngay
                                    </a>
                                </div>
                            @else
                                {{-- Trường hợp isRetriable() là true nhưng không tìm thấy route (thường không xảy ra nếu logic
                                đúng) --}}
                                <div class="alert alert-info mt-4 text-center">
                                    <p class="mb-2">Đơn hàng này đang ở trạng thái **{{ $order->status_text }}**. Vui lòng kiểm tra
                                        trạng thái sau.</p>
                                </div>
                            @endif
                        @else {{-- Các trạng thái khác không cần nút thanh toán lại (ví dụ: COD, Bank Transfer chờ admin
                            duyệt) --}}
                            <div class="alert alert-info mt-4 text-center">
                                <p class="mb-2">Đơn hàng này đang ở trạng thái **{{ $order->status_text }}**. Vui lòng kiểm tra
                                    trạng thái sau hoặc liên hệ hỗ trợ nếu cần.</p>
                            </div>
                        @endif
