                        ...
                        <div class="card h-100 {{ $address->is_default ? 'border-primary' : '' }}">
                            <div class="card-body">
                                ...
                                <p class="card-text text-muted mb-1">
                                    {{ $address->address_line }}, {{ $address->ward->name }}, {{ $address->district->name }}, {{ $address->province->name }}
                                </p>
                                ...
                                <div class="d-flex gap-2">
                                    <a href="{{ route('account.addresses.edit', $address->id) }}" class="btn btn-secondary btn-sm">Sửa</a>
                                    
                                    {{-- Nút đặt làm mặc định (chỉ hiển thị nếu chưa phải mặc định) --}}
                                    @if(!$address->is_default)
                                        <form action="{{ route('account.addresses.setDefault', $address->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <button type="button" class="btn btn-outline-danger btn-sm ms-auto" data-bs-toggle="modal" data-bs-target="#deleteAddressModal" data-delete-url="{{ route('account.addresses.destroy', $address->id) }}">
                                        Xóa
                                    </button>
                                </div>
                            </div>
